            ...
            <table>
                ...
                <tr>
                    ...
                    <div class="endpoint" id="pp_endpoint">
                        http://<?=$_SERVER['HTTP_HOST']?>/v1/Contract/privacy_policy/language/format/{format}
                    </div>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan=2>
                    <div class="form_feedback hidden" id="pp_form_feedback">
...
<br><br>
        <h3>/v1/Contract/terms_of_service/language/format/{format} - [GET]</h3>

        <form id="check_tos" action="" method="GET">
            <table>
                <tr>
                    <td colspan=2>Generated Endpoint: </td>
                </tr>
                <tr>
                    <td colspan=2>
                    <div class="endpoint" id="tos_endpoint">
                ...
                <tr>
                    <td>Format: </td>
                    <td><select name="format" id="tos_format">
                        <option value="json">json</option>
                        <?/* <option value="xml">xml</option> */?>
                        <option value="generic">generic</option>
                    <select/></td>
                ...
                <tr>
                    <td>Language: </td>
                    <td><input type="text" name="tos" id="tos" value=""></td>
                </tr>
                <tr>
                    <td colspan=2>
                    <div class="form_feedback hidden" id="tos_form_feedback">
                        <p class="bg-danger text-danger"><i>(Results displayed here)</i></p>
                    </div>
                    <div class="form_response hidden" id="tos_form_response">
                        <pre>(Response displayed here)</pre>
                    </div>
                    </td>
                </tr>
                <tr>
                    <td colspan=2><input type="button" name="submit" id="submit_tos" value="Check Phone"></td>
                </tr>
            </table>
        </form>
<br><br>
...
$(document)
    ...
    .on('click', '#submit_email', function(){
        $('#pp_form_feedback').setInfo('Getting  Privacy policy...');
        $('#pp_form_feedback').show().removeClass('hidden');
        ...
        $.get(endpoint,data,function(req) {
            if ($('#email_format').val()=='generic') {
                if (req.Success==true){
                    $('#pp_form_feedback').setSuccess(req.Message);
                } else {
                    $('#pp_form_feedback').setError(req.ErrorCode+" - "+req.Message);
                }
            }
            else if ($('#email_format').val()=='json') {
                if (req.success){
                    $('#pp_form_feedback').setSuccess(req.success);
                } else {
                    $('#pp_form_feedback').setError(req.error+" - "+req.message);
                }
            }
    ...
    .on('click', '#submit_tos', function(){
        $('#pp_form_feedback').setInfo('Getting term of condition ...');
        $('#pp_form_feedback').show().removeClass('hidden');

		var data = $('#check_tos').serializeArray();
        var endpoint = '/v1/Contract/terms_of_service/'+$('#tos').val(); //'+$('#phone').val()+'

        $('#tos_endpoint').html('http://<?=$_SERVER['HTTP_HOST']?>'+endpoint);

        $.get(endpoint,data,function(req) {
            if ($('#phone_format').val()=='generic') {
                if (req.Success==true){
                    $('#tos_form_response').setSuccess(req.Message);
                } else {
                    $('#tos_form_response').setError(req.ErrorCode+" - "+req.Message);
                }
            }
            else if ($('#phone_format').val()=='json') {
                if (req.success){
                    $('#tos_form_response').setSuccess(req.success);
                } else {
                    $('#tos_form_response').setError(req.error+" - "+req.message);
                }
            }
            var data = JSON.stringify(req, null, '\t');
            $('#tos_form_response').children().html("Response Data Size: "+data.length+"\n"+data);
		});
