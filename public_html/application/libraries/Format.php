...
class Format {
    ...
    public function to_generic($data = NULL)
    {
        if ($data === NULL && func_num_args() === 0)
        {
            $data = $this->_data;
...
//        exit(print_r($data,true));
        return json_encode(array('Data'=>(isset($data['data']))?$data['data']:'',
                        'Message'=>(isset($data['success']))?$data['success']:(isset($data['message'])?$data['message']:''),
                        'Success'=>(isset($data['success']))?'true':'false',
                        'ErrorCode'=>(isset($data['error']))?$data['error']:'0'));
    }
    // INTERNAL FUNCTIONS
